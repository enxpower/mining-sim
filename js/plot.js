// Minimal canvas line plotter: deterministic, DPI-aware, resizable.
export class Plot {
  constructor(canvas, { yMin = null, yMax = null, grid = true } = {}) {
    this.canvas = canvas;
    this.ctx = canvas.getContext("2d");
    this.series = []; // [{name,color,values:[]}]
    this.yMin = yMin;
    this.yMax = yMax;
    this.grid = grid;
  ...
  draw() {
    ...
    if (yMin == null || yMax == null) {
      ...
      for (const s of this.series) for (const v of s.values) {
        if (v == null) continue; if (v < yMin) yMin = v; if (v > yMax) yMax = v;
      }
    ...
    for (let i = 0; i <= 4; i++) {
      const v = yMin + (yMax - yMin) * i / 4; const y = (H * i) / 4;
